...
$get_record = $db->query("SELECT sj.subj_name, sc.term_two_test_one, sc.term_two_test_two, sc.term_two_exam FROM scores sc, subject_junior sj, enroll_student en WHERE en.applied_id = '{$stud_id}' AND sc.student_id = en.user_id AND sj.subj_no = sc.subject_id");
?>
...
<div class="container-fluid">
    ...
    <h4>Term Two Result. <h3>Student Name: <strong><?=cap($user_data['full_name'])?></h3></h4>
       ...
       <table class="table table-striped table-bordered table-hover table-condensed">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><?=$result['term_two_test_one']?></td>
                <td><?=$result['term_two_test_two']?></td>
                <td><strong><?=(int)$result['term_two_test_one'] + (int)$result['term_two_test_two']?></strong></td>
                <td><?=$result['term_two_exam']?></td>
                <td><strong><?=(int)$result['term_two_test_one'] + (int)$result['term_two_test_two'] + (int)$result['term_two_exam']?></strong></td>
            </tr>
            <?php endwhile;?>
        </tbody>
    </table>
