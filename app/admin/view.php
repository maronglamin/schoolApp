...
if (isset($_GET['process'])) {
    ...
    $class_id = ((isset($_POST['st_class'])) ? sanitize($_POST['st_class']) : '');
...
?>
    <div class="container-fluid">
        <div class="row">
            <div class="jumbotron">
                <h1>Process the admission details</h1>
                <p>This action enroll the student into the system.</p>
                <p><a class="btn btn-primary btn-lg" href="enrollment.php" role="button">back</a></p>
            </div>
            <div class="col-xs-10 col-xs-offset-1 col-sm-10 col-md-6 col-md-offset-3 well">
                <h3 class="text-center">The new details for the selected student</h3>
                <?php
                if (isset($_POST['save_app'])) {
                    ...
                    if (!empty($errors)) {
                        ...
                        $db->query("INSERT INTO `enroll_student` (`user_id`, `applied_id`, `check_date`, `class_id`, `app_status`) VALUES ('{$stud_id}', '{$applied_id}', '{$date_app}', '{$class_id}', '{$status}')");
                        $db->query("UPDATE `users` SET `status` = '1' WHERE `user_id` = '{$applied_id}'");
                ...
                <form action="#" method="post">
                    <div class="col-md-6 form-group">
                        ...
                        <input type="text" name="stud_num" id="stud_num" disabled class="form-control" value="<?= $stud_id ?>">
                    </div>
                    <div class="col-md-6 form-group">
                        <label for="date_touched">Date checkdate *</label>
                        <input type="date" name="date_touched" id="date_touched" class="form-control">
                    </div>
                    <div class="col-md-6 form-group">
                        <label for="status">Application Status *</label>
                        <select name="status" id="status" class="form-control">
                            <option value=""></option>
                            <option value="1">Accepted Application</option>
                            <option value="2">Reject Application</option>
                        </select>
                    </div>
                    <div class="col-md-6 form-group">
                        <label for="st_class">Select class</label>
                        <select name="st_class" id="st_class" class="form-control">
                            <option value="" <?= (isset($_POST['st_class']) == "") ?  "selected" : '' ?>></option>
                            <?php while ($classes = mysqli_fetch_assoc($class)) : ?>
                                <option value="<?= $classes['class_id'] ?>" <?= (isset($_POST['st_class']) == $classes['class_id']) ?  "selected" : '' ?>><?= $classes['grade_name'] ?></option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                    <hr>
                    <div class="col-xs-8 col-md-5">
                        <input type="submit" value="Finish" name="save_app" class="btn btn-default">
                    </div>
                </form>
            </div>
        </div>
...
if (isset($_GET['info'])) {
    $user_id = (int)sanitize($_GET['info']);
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-xs-10 col-xs-offset-1 col-sm-10 col-md-10 col-md-offset-1 well">
                ...
                <a href="enrollment.php" class="btn btn-success">Back</a>
            </div>
        </div>
    </div>
<?php }
