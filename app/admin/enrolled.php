...
$errors = [];
$enroll_get = $db->query("SELECT * FROM `users` WHERE `user_role` = '3' AND `status` = '1' ORDER BY `user_id`");
                ...
                <tbody>
                    <?php while ($applied = mysqli_fetch_assoc($enroll_get)) : ?>
                        <tr>
                            ...
                            <td><a href="detail.php?detail=<?= $applied['user_id'] ?>" class="btn btn-xs btn-info">Detail</a></td>
                        </tr>
                    <?php endwhile; ?>
                </tbody>
            </table>
